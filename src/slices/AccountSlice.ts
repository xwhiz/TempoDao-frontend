...
export const getBalances = createAsyncThunk(
  ...
  async ({ address, networkID, provider }: IBaseAddressAsyncThunk) => {
    ...
    const stakingContract = new ethers.Contract(addresses[networkID].STAKING_ADDRESS as string, OlympusStakingv2, provider);
    const warmupInfo = await stakingContract.warmupInfo(address);
    const warmupBalance = warmupInfo.deposit;
    ...
    return {
      balances: {
        ...
        warmupOhm: ethers.utils.formatUnits(warmupBalance, "gwei"),
...
export const loadAccountDetails = createAsyncThunk(
  ...
  async ({ networkID, provider, address }: IBaseAddressAsyncThunk) => {
    ...
    if (addresses[networkID].STAKING_ADDRESS) {
      const stakingContract = new ethers.Contract(addresses[networkID].STAKING_ADDRESS as string, OlympusStakingv2, provider);
      const warmupInfo = await stakingContract.warmupInfo(address);
      warmupBalance = warmupInfo.deposit;
    }
    /*
