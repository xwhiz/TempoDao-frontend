...
import { abi as Treasuryv2 } from "../abi/OlympusTreasury.json";
...
`;
    ...
    const circSupply = (await sohmMainContract.circulatingSupply()) / Math.pow(10, 9);
    const totalSupply = (await ohmContract.totalSupply()) / Math.pow(10, 9);
    const marketCap = totalSupply*marketPrice;


    const treasuryMarketValue = 1;
    ...
    const fiveDayRate = Math.pow(1 + stakingRebase, 5 * 1.5) - 1;
    const stakingAPY = Math.pow(1 + stakingRebase, 365 * 1.5) - 1;
