...
export const PoolDeposit = props => {
  ...
  useEffect(() => {
    props.setInfoTooltipMessage([
      "Deposit sTEMPO to win! Once deposited, you will receive a corresponding amount of 33T and be entered to win until your sTEMPO is withdrawn.",
    ]);
  }, []);
          ...
          <Box display="flex" alignItems="center" flexDirection={`${isMobileScreen ? "column" : "row"}`}>
            ...
            {address && hasAllowance("sohm") ? (
              <Button
                ...
                fullWidth
              >
                {txnButtonText(pendingTransactions, "pool_deposit", "Deposit sTEMPO")}
              </Button>
            ) : (
              <Button
                className="pool-deposit-button"
            ...
            <div className="data-row">
              ...
              <Typography variant="body1" align="right">
                {isAppLoading ? (
                  ...
                  <>{new Intl.NumberFormat("en-US").format(sohmBalance)} sTEMPO</>
                )}
              </Typography>
            </div>
          </div>
